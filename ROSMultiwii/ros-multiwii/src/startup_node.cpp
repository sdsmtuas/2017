...
public:
    ...
    void setup() {
        ...
        // subscriber
        rc_in_sub = nh.subscribe("rc/override", 1, &StartupNode::rc_override, this);
        motor_control_sub = nh.subscribe("actuator_control", 1, &StartupNode::motor_control, this);

        // services
        arming_srv = nh.advertiseService("cmd/arming", &StartupNode::arming, this);
        send_msg_srv = nh.advertiseService("cmd/send_msg", &StartupNode::send_raw_msg, this);
        receive_msg_srv = nh.advertiseService("cmd/receive_msg", &StartupNode::receive_raw_msg, this);
    }
    ...
    void setDynamicConfigureCallback() {
        // dynamic configure
        dyn_conf_srv.setCallback(boost::bind(&StartupNode::dynconf_callback, this, _1, _2));
    }
...
int main(int argc, char **argv) {
    ...
    node.fc().subscribe(&StartupNode::onImu, &node);
    node.fc().subscribe(&StartupNode::onAttitude, &node);
    node.fc().subscribe(&StartupNode::onAltitude, &node);
    node.fc().subscribe(&StartupNode::onRc, &node);
    node.fc().subscribe(&StartupNode::onServo, &node);
    node.fc().subscribe(&StartupNode::onMotor, &node);
    node.fc().subscribe(&StartupNode::onMisc, &node);
    node.fc().subscribe(&StartupNode::onAnalog, &node);
    node.fc().subscribe(&StartupNode::onStatus, &node);
